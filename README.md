...
Other tiles gain native controls when their rendered video reaches 640 pixels wide. They retain them down to 560 pixels. Size-driven changes wait 300 milliseconds after resizing stops. Twitch accepts the controls option only at construction, so only players whose control mode changes are recreated, preserving their settings and quality.

Paused tiles release their embed and show a stream image. Hovering a small paused tile can start a muted preview, which is released on leaving. Single, spotlight, and expanded tiles stay paused until explicitly resumed. Clicking a paused still resumes playback; spotlight is controlled by its header button.
