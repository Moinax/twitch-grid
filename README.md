...
The sidebar includes Video player, Global latency and Spotlight latency settings. The custom player is the default and renders both tiles and sidebar previews as HTML5 video with HLS.js; Twitch embed stays selectable. Stable latency preserves the current safety margin, while Low latency stays closer to the live edge and gently speeds up to catch up. The global latency applies to grid tiles and defaults to stable; the spotlight latency applies to the stream in front, or to a lone tile, and defaults to low. A tile can pick its own latency from the player's resolution menu until either setting changes. Twitch's embed does not expose this choice, so the latency settings are disabled for it. These choices persist in this browser. Switching the player recreates active players while retaining tile order, volume, mute, pause, spotlight, and chat preferences; a latency change retunes the running players in place, so a stream keeps its video and sound when it takes or leaves the spotlight.

The custom player draws its own bar in the site's theme instead of the browser's video controls, at the bottom right of the video. At rest it is a small badge with the rendered resolution and an icon telling whether the level is automatic or fixed by hand; a moving pointer over the video grows it into the bar with play and pause, sound with its volume slider, the spotlight, and fullscreen, which takes the whole tile so its header stays reachable. A pointer left still for three seconds lets it shrink back. The tile header drops its own play, sound, spotlight and in-window expand buttons for custom players, leaving the chat, the collaboration menu and the close button. The resolution opens a menu listing the HLS levels, the latency for that tile and the switch to Twitch's embed, placed above or below the badge wherever there is room, with the latency at the far end. Under the global mute the bar's sound button and volume slider are disabled. The bar drives the video element, and the tile header reads the change back. Controls can change on resize without replacing the video. A tile on Twitch's embed keeps a header shortcut back to the custom player; a custom tile switches from its resolution menu. Pausing a custom player keeps its last frame and video element, including global pause. The usual dimmed overlay displays a centered Play button to resume the tile. Hovering a paused tile does not resume it. Tiles restored in a paused state keep a poster until first playback. HLS.js selects quality automatically until a level is chosen, caps automatic selection to the rendered video size, and reapplies a manually selected level once the levels are known. Failed playback offers a retry and a switch to the Twitch embed; the setting also allows it at any time.
...
The spotlight takes the whole width of the grid, the other tiles in a strip under it, or the whole height, the other tiles in a column beside it, whichever renders the wider video. Those tiles then fill the room the spotlight cannot use, in as many columns as fit while staying 290 pixels wide, the width a player's bar needs; the layout shows as many of them as that room holds and never narrows the spotlight to fit more. A chat open on the spotlight is served from that same room before the small tiles take it, and settles on the side that costs the video the least.
