...
All browsers use the same video rendering. The previous rendering and comparison selector have been removed; previously saved rendering choices no longer affect playback.
...
Each tile header has playback and sound controls. The horizontal volume slider opens only while hovering over the sound control or slider. Sound icons reflect the current player state, while saved audio intent survives reloads. The global playback button pauses or resumes the grid; each tile can then override that choice. Global mute disables individual sound changes until it is released. Controls retain accessible labels without tooltips over the players. Dragging a header reorders the grid; dropping onto the spotlight exchanges their roles.
...
Paused tiles show a stream image instead of keeping an idle embed. Hovering a small paused tile can start a muted preview; leaving releases it. Spotlight, expanded and single tiles stay paused until explicitly resumed. Loading and offline placeholders sit below the iframe. A stalled player shows its status in the header and accepts clicks inside the native Twitch player.
