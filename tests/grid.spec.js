...
test('the spotlight button and a click on a small tile bring one stream in front with the sound', async ({ page }) => {
  ...
  await three.hover(); await three.locator('.spotlight').click();
  ...
  await expect(three.locator('.spotlight')).toHaveAttribute('aria-label', 'Revenir à la grille (SHIFT+CLICK)');
  await expect(one.locator('.spotlight')).toHaveAttribute('aria-label', 'Spotlight (SHIFT+CLICK)');
  ...
  await one.hover(); await one.locator('.spotlight').click();
  expect(await page.evaluate(() => focused)).toBe('one');
  await two.hover(); await two.locator('.spotlight').click();
...
test('the mute-all button silences every stream and gives the sound back to those that had it', async ({ page }) => {
  ...
  await expect(button).toHaveAttribute('aria-label','Réactiver le son (Shift+M)');
  ...
  await page.locator('#grid [data-login="three"]').hover(); await sound.hover();
  ...
  await page.locator('#grid [data-login="three"]').hover(); await page.locator('#grid [data-login="three"] .spotlight').click();
  ...
  await page.locator('#grid [data-login="one"]').hover(); await page.locator('#grid [data-login="one"] .spotlight').click();   // three leaves and gives back its silence
  expect(await state()).toEqual({ mutedAll: ['one', 'two'], muted: [true, true, true] });
...
test('a miniature plays and pauses on click and fullscreens on double click', async ({ page }) => {
  ...
  await player.click();   // the surface only resumes: a playing tile stays as it is
  await expect(tile.locator('.bar .pp')).toHaveAttribute('aria-pressed', 'false');
  await tile.hover(); await tile.locator('.bar .pp').click();
...
for (const connected of [false,true]) test(`collaboration icons and participant additions preserve grid and audio in ${connected?'connected':'guest'} mode`,async({page})=>{
  ...
  await expect(page.locator('.chat iframe')).toHaveCount(0);
  await page.keyboard.press('Escape');
  await page.locator('#grid [data-login="partner"]').hover();
...
test('language and theme changes persist without recreating video players',async({page})=>{
  ...
  await page.goto('/');
  await expect(page.locator('#grid .tile')).toHaveClass(/full-player/);await readyPlayers(page);   // the lone tile gets its controls once measured; capture the player after that
  await page.evaluate(()=>window.settingsPlayer=tiles.get('one').player);
...
for (const connected of [false,true]) test(`paused ${connected?'automatic follows':'guest'} grid loads thumbnails without constructing players`, async ({page}) => {
  ...
  await page.addInitScript(({connected,channels})=>{
    if(connected) {
      sessionStorage.setItem('tg.session',JSON.stringify('valid'));
      localStorage.setItem('tg.grids.connected',JSON.stringify({activeId:'live-follows',items:[{id:'default',layout:{order:[]}},{id:'live-follows',layout:{order:[],locked:true,allPaused:true}}]}));
    } else {
...
test('dynamic grid preserves manual settings on return',async({page})=>{
  ...
  await page.clock.runFor(1500);
  const result=await page.evaluate(()=>({before:{order:window.before.order,paused:window.before.paused.live0??false,volume:window.before.volume.live0,muted:window.before.muted.live0},after:{order,paused:tilePaused(tiles.get('live0')),volume:tiles.get('live0').volume,muted:tiles.get('live0').muted}}));
...
test('global playback and mute shortcuts ignore editing and preserve mute after Shift release', async ({page}) => {
  const errors = await setup(page);
  await page.addInitScript(() => localStorage.setItem('tg.layout.guest', JSON.stringify({order:['one','two'],muted:{one:true,two:false}})));
  await page.goto('/');
  await page.locator('#audio-overlay').click();
  ...
  const play = page.locator('#playall'), mute = page.locator('#muteall');
  ...
  await expect(mute).toHaveAttribute('aria-pressed','true');
  await expect(page.locator('#soundfollow')).toHaveAttribute('aria-pressed','false');
  await expect.poll(() => page.evaluate(() => [...tiles.values()].every(t => t.player?.getMuted() === true))).toBe(true);
  await page.keyboard.press('Shift+M');
  await expect(mute).toHaveAttribute('aria-pressed','false');
  await expect.poll(() => page.evaluate(() => tiles.get('two').player?.getMuted())).toBe(false);
  await expect(play).toHaveAttribute('data-tip', /Espace/);
  await expect(mute).toHaveAttribute('data-tip', /Shift\+M/);
  await page.locator('#q').focus();
  await page.keyboard.press('Space');
  await page.keyboard.press('Shift+M');
  await expect(page.locator('#q')).toHaveValue(' M');
  await expect(play).toHaveAttribute('aria-pressed','false');
  await expect(mute).toHaveAttribute('aria-pressed','false');
  await page.evaluate(() => {
    const field = document.createElement('div');
    field.contentEditable = 'true';
    field.id = 'shortcut-editor';
  ...
  await expect(play).toHaveAttribute('aria-pressed','false');
  await expect(mute).toHaveAttribute('aria-pressed','false');
  await page.locator('#shortcut-editor').evaluate(el => el.remove());
  expect(errors).toEqual([]);
});

test('spotlight chat defaults allow tile overrides and settings replace them', async ({page}) => {
  const errors = await setup(page); await mockChat(page);
  await page.setViewportSize({width:1600,height:1000});
  await page.addInitScript(() => {
    if (!localStorage.getItem('tg.layout.guest')) localStorage.setItem('tg.layout.guest', JSON.stringify({order:['one','two'],focused:'one'}));
  });
  await page.goto('/'); await readyPlayers(page);
  const one = page.locator('#grid [data-login="one"]'), two = page.locator('#grid [data-login="two"]');
  await expect(one.locator('.chat')).toBeHidden();
  await choose(page, '#spotlight-chat-setting', 'on');
  await expect(one.locator('.chat')).toBeVisible();
  await expect(two.locator('.chat')).toBeHidden();
  await choose(page, '#spotlight-chat-position-setting', 'left');
  await expect(one.locator('.tile-body')).toHaveAttribute('data-chat-position', 'left');
  await one.locator('.chat-toggle').click();
  await expect(one.locator('.chat')).toBeHidden();
  await two.hover();
  await two.locator('.spotlight').click();
  await expect(two.locator('.chat')).toBeVisible();
  await expect(two.locator('.tile-body')).toHaveAttribute('data-chat-position', 'left');
  await one.locator('.spotlight').click();
  await expect(one.locator('.chat')).toBeHidden();
  await choose(page, '#spotlight-chat-position-setting', 'bottom');
  await expect(one.locator('.chat')).toBeVisible();
  await expect(one.locator('.tile-body')).toHaveAttribute('data-chat-position', 'bottom');
  await page.reload(); await readyPlayers(page);
  if (await page.locator('#audio-overlay').isVisible()) await page.locator('#audio-overlay').click();
  await expect(one.locator('.chat')).toBeVisible();
  await expect(one.locator('.tile-body')).toHaveAttribute('data-chat-position', 'bottom');
  await selectChatPosition(one, 'right');
  await choose(page, '#spotlight-chat-setting', 'off');
  await expect(one.locator('.chat')).toBeHidden();
  await two.hover();
  await two.locator('.spotlight').click();
  await expect(two.locator('.chat')).toBeHidden();
...
test('a minute counts for less in a muted corner of the grid than in the spotlight', async ({ page }) => {
  ...
  await page.locator('#grid [data-login="star"]').hover();
